            ...
            <div class="card-body mt-3">
                <div class=" card-title ms-2">
                    <h5 style="margin-bottom: 20px;">
                        {{ isset($incomingSupply) ? 'Ubah Data Barang Masuk' : 'Tambah Data Barang Masuk' }}</h5>
                ...
                <div>
                    ...
                    <!-- START FORM -->
                    <form
                        action="{{ isset($incomingSupply) ? route('barang_masuk.tambah.update', $incomingSupply->id) : route('barang_masuk.tambah.simpan') }}"
                        ...
                        <div class="my-3 p-3 bg-body rounded shadow-sm">
                                ...
                                <select class="input-select2 form-control" name="master_goods_id" id="master_goods_id"
                                    ...
                                    @foreach ($data['goods'] as $barang)
                                        <option value="{{ $barang->id }}"
                                            @if (isset($incomingSupply)) {{ $incomingSupply->Supplies->master_goods_id == $barang->id ? 'selected' : '' }} @endif>
                            ...
                            <div class="form-floating mb-3 form-group ">
                                <input type="number" class="form-control" placeholder="Jumlah" name="quantity"
                                    id="quantity"
                                    value="{{ isset($incomingSupply) ? $incomingSupply->quantity_in : old('quantity_in') }}"
                                    required>
                            ...
                            <div class="form-floating mb-3 form-group ">
                                <input type="text" class="form-control" placeholder="Nama Anda" name="operator"
                                    id="operator"
                                    value="{{ isset($incomingSupply) ? $incomingSupply->operator : old('operator') }}"
                                    required>
                                <label for="floatingInput operator">Nama Anda</label>
                            </div>
                            <div class="form-floating mb-3 form-group ">
                                <input type="date" class="form-control" name="entry_date" id="entry_date"
                                    value="{{ isset($incomingSupply) ? $incomingSupply->entry_date : old('entry_date') }}"
                                    required>
